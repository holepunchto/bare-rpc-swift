...
The full Swift test suite runs without any external setup. Two tests in `BareInteropTests` additionally spawn a live JavaScript peer; without a peer they print a skip notice and pass.
...
Both prerequisites are wired up automatically in CI. CI runs with `CI=true`, which turns missing prerequisites into hard failures so a misconfigured runner can't silently no-op past the live tests.
